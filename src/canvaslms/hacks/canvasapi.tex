...
\nwalsodefined{\\{NW4Ar8dO-nRuDO-2}\\{NW4Ar8dO-nRuDO-3}\\{NW4Ar8dO-nRuDO-4}\\{NW4Ar8dO-nRuDO-5}\\{NW4Ar8dO-nRuDO-6}\\{NW4Ar8dO-nRuDO-7}\\{NW4Ar8dO-nRuDO-8}\\{NW4Ar8dO-nRuDO-9}\\{NW4Ar8dO-nRuDO-A}\\{NW4Ar8dO-nRuDO-B}}\nwused{\\{NW4Ar8dO-3Q57f0-1}}\nwendcode{}\nwbegindocs{8}\nwdocspar
...
{\Tt{}get{\_}assignment{\_}groups()\nwendquote} but no {\Tt{}get{\_}assignment{\_}group()\nwendquote}).
For these, we use {\Tt{}include{\_}singular=False\nwendquote}.
...
We capture {\Tt{}attr{\_}name\nwendquote} as a closure variable on line~358.
This is crucial for correct decorator behavior when multiple decorators are
applied to the same class.
We must \emph{not} store {\Tt{}attr{\_}name\nwendquote} as an instance attribute (like
{\Tt{}self.attr{\_}name\nwendquote}) because that would be shared across all decorator
applications.
For example, if we decorate {\Tt{}Course\nwendquote} with both {\Tt{}CacheGetMethods("assignment")\nwendquote}
and {\Tt{}CacheGetMethods("user")\nwendquote}, storing as {\Tt{}self.attr{\_}name\nwendquote} would cause the
second decorator to overwrite the first decorator's value, making both use
``user'' incorrectly.
By using the closure variable {\Tt{}attr{\_}name\nwendquote}, each decorator captures its own
...
\begin{enumerate}
\item \textbf{Bulk fetch path} ({\Tt{}if\ not\ attr{\_}all{\_}fetched\nwendquote}): When cache is
...
\item \textbf{Cached path} ({\Tt{}else\nwendquote}): When cache is already populated and
...
\end{enumerate}
The {\Tt{}else\nwendquote} is critical to avoid the double-yield bug: without it, we would
...
Note that we use {\Tt{}cache{\_}attr{\_}name\nwendquote} as the loop variable, not {\Tt{}attr{\_}name\nwendquote}.
This is critical to avoid shadowing the closure variable {\Tt{}attr{\_}name\nwendquote} that was
captured in line~371.
If we used {\Tt{}attr{\_}name\nwendquote} as the loop variable, Python would treat it as a local
variable throughout the entire {\Tt{}new{\_}get{\_}attrs\nwendquote} function, causing an
{\Tt{}UnboundLocalError\nwendquote} when we try to access it before the loop (in
lines~494--495).
...
try:
  if obj.grade not in NOREFRESH_GRADES:
    \LA{}check if submission TTL has expired~{\nwtagstyle{}\subpageref{NW4Ar8dO-2zKIzo-1}}\RA{}
except AttributeError:
  pass
...
expired.
If the object has no {\Tt{}{\_}fetched{\_}at\nwendquote} attribute, we treat it as outdated to
ensure it gets refreshed.
\nwenddocs{}\nwbegincode{62}\sublabel{NW4Ar8dO-2zKIzo-1}\nwmargintag{{\nwtagstyle{}\subpageref{NW4Ar8dO-2zKIzo-1}}}\moddef{check if submission TTL has expired~{\nwtagstyle{}\subpageref{NW4Ar8dO-2zKIzo-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NW4Ar8dO-1RGqgt-1}}\nwenddeflinemarkup
try:
  if datetime.now() - obj._fetched_at > timedelta(minutes=5):
    return True
except AttributeError:
  return True
\nwused{\\{NW4Ar8dO-1RGqgt-1}}\nwendcode{}\nwbegindocs{63}\nwdocspar
...
\nwenddocs{}\nwbegincode{72}\sublabel{NW4Ar8dO-2ap6LS-2}\nwmargintag{{\nwtagstyle{}\subpageref{NW4Ar8dO-2ap6LS-2}}}\moddef{if statements for resetting \code{}{\_}all{\_}fetched\edoc{} for various attributes~{\nwtagstyle{}\subpageref{NW4Ar8dO-2ap6LS-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NW4Ar8dO-2m2V3r-1}}\nwprevnextdefs{NW4Ar8dO-2ap6LS-1}{\relax}\nwenddeflinemarkup
...
The Canvas API only provides {\Tt{}get{\_}assignment{\_}groups()\nwendquote} (plural), not a
singular {\Tt{}get{\_}assignment{\_}group()\nwendquote} method.
We use {\Tt{}CacheGetMethods\nwendquote} with {\Tt{}include{\_}singular=False\nwendquote} to cache only the
plural method.
\nwenddocs{}\nwbegincode{80}\sublabel{NW4Ar8dO-nRuDO-8}\nwmargintag{{\nwtagstyle{}\subpageref{NW4Ar8dO-nRuDO-8}}}\moddef{functions~{\nwtagstyle{}\subpageref{NW4Ar8dO-nRuDO-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NW4Ar8dO-3Q57f0-1}}\nwprevnextdefs{NW4Ar8dO-nRuDO-7}{NW4Ar8dO-nRuDO-9}\nwenddeflinemarkup
...
Similar to assignment groups, the Canvas API only provides {\Tt{}get{\_}modules()\nwendquote}
(plural), so we cache with {\Tt{}include{\_}singular=False\nwendquote}.
\nwenddocs{}\nwbegincode{82}\sublabel{NW4Ar8dO-nRuDO-9}\nwmargintag{{\nwtagstyle{}\subpageref{NW4Ar8dO-nRuDO-9}}}\moddef{functions~{\nwtagstyle{}\subpageref{NW4Ar8dO-nRuDO-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NW4Ar8dO-3Q57f0-1}}\nwprevnextdefs{NW4Ar8dO-nRuDO-8}{NW4Ar8dO-nRuDO-A}\nwenddeflinemarkup
...
infrequently.
The Canvas API only provides plural methods ({\Tt{}get{\_}group{\_}categories()\nwendquote},
{\Tt{}get{\_}groups()\nwendquote}), not singular methods.
Note that groups can be fetched from both {\Tt{}Course\nwendquote} and {\Tt{}GroupCategory\nwendquote}
objects, so we cache on both.
For {\Tt{}group{\_}category\nwendquote}, the plural is irregular (``categories'' not
``categorys''), so we specify {\Tt{}plural{\_}name="group{\_}categories"\nwendquote}.
\nwenddocs{}\nwbegincode{84}\sublabel{NW4Ar8dO-nRuDO-A}\nwmargintag{{\nwtagstyle{}\subpageref{NW4Ar8dO-nRuDO-A}}}\moddef{functions~{\nwtagstyle{}\subpageref{NW4Ar8dO-nRuDO-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NW4Ar8dO-3Q57f0-1}}\nwprevnextdefs{NW4Ar8dO-nRuDO-9}{NW4Ar8dO-nRuDO-B}\nwenddeflinemarkup
...
user.
\nwenddocs{}\nwbegincode{86}\sublabel{NW4Ar8dO-nRuDO-B}\nwmargintag{{\nwtagstyle{}\subpageref{NW4Ar8dO-nRuDO-B}}}\moddef{functions~{\nwtagstyle{}\subpageref{NW4Ar8dO-nRuDO-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NW4Ar8dO-3Q57f0-1}}\nwprevnextdefs{NW4Ar8dO-nRuDO-A}{\relax}\nwenddeflinemarkup
...
\nwixlogsorted{c}{{check if submission TTL has expired}{NW4Ar8dO-2zKIzo-1}{\nwixu{NW4Ar8dO-1RGqgt-1}\nwixd{NW4Ar8dO-2zKIzo-1}}}%
