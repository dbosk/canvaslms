...
singular method, so we cannot use {\Tt{}CacheGetMethods\nwendquote} which expects both.
For now, we skip caching assignment groups.
TODO: Implement a simpler caching decorator for plural-only methods.
\nwenddocs{}\nwbegindocs{80}Modules represent the learning structure of a course.
They're used for filtering assignments and rarely change after initial setup.
However, similar to assignment groups, the Canvas API only provides
{\Tt{}get{\_}modules()\nwendquote} (plural), so we skip caching for now.
\nwenddocs{}\nwbegindocs{81}\nwdocspar
...
infrequently.
However, the Canvas API only provides plural methods ({\Tt{}get{\_}group{\_}categories()\nwendquote},
{\Tt{}get{\_}groups()\nwendquote}), not singular methods.
We skip caching these for now until we implement a plural-only caching decorator.
\nwenddocs{}\nwbegindocs{82}\nwdocspar
